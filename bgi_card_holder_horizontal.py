...
def bgi_divisor_node_group():
	bgi_divisor = bpy.data.node_groups.new(type = 'GeometryNodeTree', name = "BGI_Divisor")
...
def bgi_card_holder_horizontal():
	if "BGI_Divisor" not in bpy.data.node_groups:
		bgi_divisor = bgi_divisor_node_group()
	else:
		bgi_divisor = bpy.data.node_groups["BGI_Divisor"]
